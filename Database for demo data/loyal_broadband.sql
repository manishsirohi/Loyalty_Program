...
CREATE TABLE `broadband` (
  ...
  `Broadband_No` varchar(45) NOT NULL,
  `BB_amount` varchar(20) DEFAULT NULL,
  `Date` timestamp(4) NULL DEFAULT NULL,
  PRIMARY KEY (`Broadband_No`),
  KEY `broadband_foreign` (`id_no`),
  CONSTRAINT `broadband_foreign` FOREIGN KEY (`id_no`) REFERENCES `user` (`id_no`) ON DELETE NO ACTION ON UPDATE NO ACTION
